...
**Adapt to a venue / column width** — for a narrow single column, shrink
`\trapW` (2.6→1.8), `\blockH` (3.0→2.2), and `\gap` (0.9→0.6); or wrap the whole
`tikzpicture` in `\resizebox{\columnwidth}{!}{...}` (needs `graphicx` in the
paper). For full-width, increase the same values.
