...
\begin{tikzpicture}[
  ...
  \node[compbox] (client) at (0,0) {Client};
  \node[icon] (cloud) at (3.0,0) {\cloudpic};
  \node[slbl, below=0pt of cloud] {Internet};
  \node[compbox] (lb) at (6.0,0) {Load\\balancer};

  % GPU-backed inference nodes (server + gpu composed in one box)
  \node[nodebox] (node1) at (9.4,1.5) {\serverpic\hspace{4pt}\gpupic};
  \node[nodebox] (node2) at (9.4,-1.5) {\serverpic\hspace{4pt}\gpupic};
  \node[slbl, below=1pt of node1] (n1lab) {Inference node 1};
  \node[slbl, below=1pt of node2] (n2lab) {Inference node 2};

  \node[icon] (db) at (13.0,0) {\dbpic};
  \node[slbl, below=2pt of db] {Feature store};
  ...
  \node[slbl, text=otteal!70!black] at (11.55,0.55) {features};
  ...
  \begin{scope}[on background layer]
    ...
    \node[slbl, anchor=south west] at ([xshift=2pt, yshift=2pt]cluster.north west) {Inference cluster};
  \end{scope}

  % title
  \node[font=\sffamily\bfseries] at (6.5,3.2) {Model inference serving};
