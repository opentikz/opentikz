...
\begin{tikzpicture}[
  ...
  \node[slbl, below=2pt of data] {Training data};

  % GPU workers
  \node[icon] (w1) at (4.3,2.5)  {\gpupic};
  \node[icon] (w2) at (4.3,0)    {\gpupic};
  \node[icon] (w3) at (4.3,-2.5) {\gpupic};
  \node[slbl, below=2pt of w1] {Worker 1};
  \node[slbl, below=2pt of w2] {Worker 2};
  \node[slbl, below=2pt of w3] {Worker 3};

  % parameter server
  \node[icon] (ps) at (9.2,0) {\serverpic};
  \node[slbl, below=2pt of ps] {Parameter server};

  % data sharding: dataset -> workers
  \draw[data] (data.east) -- (w1.west);
  \draw[data] (data.east) -- (w2.west) node[midway, above, slbl] {batch};
  ...
  \node[slbl, text=otpurple!85!black] at (6.75,0.6)  {$\nabla\theta$};
  \node[slbl, text=otteal!70!black]   at (6.75,-0.6) {$\theta$};

  % title
  \node[font=\sffamily\bfseries] at (4.6,3.7) {Distributed data-parallel training};
